...
namespace KaiGen\Tests;

// Only run in test environment.
$kaigen_host = isset( $_SERVER['HTTP_HOST'] ) ? sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] ) ) : '';
if ( ! defined( 'E2E_TESTING' ) && ! str_contains( $kaigen_host, '127.0.0.1:9400' ) && ! str_contains( $kaigen_host, 'localhost:9400' ) ) {
	return;
}

// Log that we're in test mode.
// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
error_log( 'KaiGen E2E Mock: Test environment detected - E2E_TESTING: ' . ( defined( 'E2E_TESTING' ) ? 'true' : 'false' ) . ', Host: ' . ( isset( $_SERVER['HTTP_HOST'] ) ? sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] ) ) : 'unknown' ) );
...
add_filter(
	...
	function ( $pre, $args, $url ) {
		...
		// Mock xAI chat completions endpoint (for alt text generation).
		if ( str_contains( $url, 'api.x.ai/v1/chat/completions' ) ) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			error_log( 'KaiGen E2E Mock: Returning mocked xAI chat completion output' );
				...
				'body'     => wp_json_encode(
					[
						'choices' => [
							[
								'message' => [
									'content' => 'A tiny test image with a simple green checkmark icon.',
								],
							],
						],
					]
...
add_action(
	'template_redirect',
	function () {
		$kaigen_url = isset( $_SERVER['REQUEST_URI'] ) ? esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : '';

		if ( str_contains( $kaigen_url, '/wp-content/uploads/' ) && ( str_ends_with( $kaigen_url, '.webp' ) || str_ends_with( $kaigen_url, '.png' ) ) ) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			error_log( 'KaiGen E2E Mock (template_redirect): Serving mocked image for uploaded file request: ' . $kaigen_url );
