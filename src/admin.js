...
document.addEventListener( 'DOMContentLoaded', function () {
	...
	function handleProviderChange() {
		...
		if ( selectedProvider ) {
			const selectedApiKeyInput = document.getElementById(
				'kaigen_' + selectedProvider + '_api_key'
			);
	...
	function checkProviderWarning() {
		...
		if ( selectedProvider ) {
			...
			if ( apiKeyInput && ! apiKeyInput.value.trim() ) {
				...
				warning.className =
					'notice notice-warning inline kaigen-warning';
				warning.innerHTML =
					'<p><strong>Warning:</strong> You have selected ' +
					getProviderName( selectedProvider ) +
					' but no API key is set. Please enter your API key below.</p>';
				...
				if ( providerRow ) {
					...
					providerRow.parentNode.insertBefore(
						warningRow,
						providerRow.nextSibling
					);
				}
			}
		}
