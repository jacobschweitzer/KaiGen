 ...
 * @return {JSX.Element} The enhanced MediaUpload component with the AITab.
 */
addFilter(
	'editor.MediaUpload',
	'kaigen/add-ai-tab',
	( OriginalMediaUpload ) => {
		...
		return ( props ) => {
			...
			const isImageBlock =
				selectedBlock && selectedBlock.name === 'core/image';

			/**
			 * Checks if the current block already has image data.
			 *
			 * @return {boolean} True if the block has an image, otherwise false.
			 */
			const hasImageData = () => {
				return (
					selectedBlock &&
