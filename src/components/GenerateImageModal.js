...
import { Button, TextareaControl, Modal, Spinner, Dropdown, Dashicon } from '@wordpress/components';
 ...
 */
const GenerateImageModal = ({ isOpen, onClose, onSelect, initialReferenceImage }) => {
				...
				<Dropdown
					...
					renderContent={() => {
						...
						const allImages = initialReferenceImage 
							? [initialReferenceImage, ...referenceImages.filter(img => img.id !== initialReferenceImage.id)]
							: referenceImages;
											...
											<img
												...
												onClick={() => {
													setSelectedRefs(prev => {
														...
														if (isSelected) {
															return prev.filter(s => s.url !== img.url);
												...
												className={`kaigen-modal-reference-image ${selectedRefs.some(s => s.url === img.url) ? 'kaigen-modal-reference-image-selected' : ''}`}
											/>
										))}
									</div>
								) : (
